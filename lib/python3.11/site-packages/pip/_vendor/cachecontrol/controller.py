...
class CacheController(object):
    ...
    def cache_response(self, request, response, body=None, status_codes=None):
        ...
        no_store = False
        if "no-resource" in cc:
            no_store = True
            logger.debug('Response header has "no-resource"')
        if "no-resource" in cc_req:
            no_store = True
            logger.debug('Request header has "no-resource"')
        if no_store and self.cache.get(cache_url):
            logger.debug('Purging existing cache entry to honor "no-resource"')
            self.cache.delete(cache_url)
        if no_store:
            return
